...
class ApicalTiebreakTemporalMemory(object):
  ...
  def __init__(self,
    ...
    self.bestWinnerCells =np.empty(0, dtype="uint32")
  ...
  def visualize(self):
    """
    return state of TM as a numpy array which we can visualize using opencv
    """
    state = np.zeros([self.cellsPerColumn,self.columnCount,3])
    #ACTIVE CELLS IN RED
    for activeCell in self.activeCells:
      column = activeCell/self.cellsPerColumn
      cell = activeCell % self.cellsPerColumn
      state[cell,column] += (0,0,255) #BGR
    #PREDICTED CELLS IN BLUE
    for predictedCell in self.predictedCells:
      column = predictedCell/self.cellsPerColumn
      cell = predictedCell % self.cellsPerColumn
      state[cell,column] += (255,0,0) #BGR
    #WINNER CELLS IN GREEN
    for winnerCell in self.winnerCells:
      column = winnerCell/self.cellsPerColumn
      cell = winnerCell % self.cellsPerColumn
      state[cell,column] += (0,255,0) #BGR
    return state
  ...
  def activateCells(self,
    ...
    if self.verbosity > 0:
      print "learningCells:" + str(learningCells)
     
    #apicalWinnerCells = self.selectBestWinnerCells(learningCells,[])
    #and then use apicalWinnerCells instead of correctPredictedCells
    #NOTE: Originally they learned apical on all 'learningCells'
    #Changed it to only correct predicted cells or best winning?
    (learningActiveApicalSegments,
     ...
     newApicalSegmentCells) = self._calculateApicalLearning(
       ...
       self.matchingApicalSegments, self.apicalPotentialOverlaps)
    if self.verbosity > 0:
      print "newApicalSegmentCells:" + str(newApicalSegmentCells)
    ...
    if learn:
      ...
      for learningSegments in (learningActiveBasalSegments,
                               learningMatchingBasalSegments):
        #Something will go wrong here if using reduced basal segments
        #print "learning basal"
...
#         print "learning new basal segment"
#         print newBasalSegmentCells
#         print basalGrowthCandidates
#         
        self._learnOnNewSegments(self.basalConnections, self.rng,
                                 newBasalSegmentCells, basalGrowthCandidates,
                                 self.initialPermanence, self.sampleSize,
                                 self.maxSynapsesPerSegment)
        #print "end learning new basal segment"
    ...
    #self.bestWinnerCells = self.selectBestWinnerCells(learningCells,newBasalSegmentCells)
    self.predictedActiveCells = correctPredictedCells
    
  def selectBestWinnerCells(self,winnerCells,newSegmentCells):
    ''' New segment cells are always the only cells in the column by definition. Only check
    other cells and select the best among the column'''
    winnerCells = np.setdiff1d(winnerCells,newSegmentCells)
    allSegmentsOfWinnerCells = []
    for winnerCell in winnerCells:
      allSegmentsOfWinnerCells.extend(self.basalConnections.getSegmentsForCell(winnerCell))
    np.asarray(allSegmentsOfWinnerCells)
    segments = self._chooseBestSegmentPerColumn(self.basalConnections, winnerCells,
                                                allSegmentsOfWinnerCells, self.basalPotentialOverlaps,
                                                self.cellsPerColumn)
    bestWinnerCells = self.basalConnections.mapSegmentsToCells(segments)
    result = np.concatenate((bestWinnerCells,newSegmentCells))
    #assert(len(result) == 2) #this use case
    return result
  ...
  def _calculateApicalLearning(self,
    ...
    #Punish all matching connections that are not learning, also in within the column!
    apicalMatchingCells = self.apicalConnections.mapSegmentsToCells(matchingApicalSegments)
    #apicalActiveCells = self.apicalConnections.mapSegmentsToCells(activeApicalSegments)
    punishedCells = np.setdiff1d(apicalMatchingCells,learningCells)
    apicalSegmentsToPunish = self.apicalConnections.filterSegmentsByCell(matchingApicalSegments,
                                                                         punishedCells)

    #apicalSegmentsToPunish = matchingApicalSegments[~correctMatchingApicalMask]
    
    #print "number of segments punished: " + str(len(apicalSegmentsToPunish))
  ...
  def _calculatePredictedCells(self, activeBasalSegments, activeApicalSegments):
    ...
    if self.useApicalTiebreak == False:
      #print "my predicted cells:"
      predictedCells = np.unique(np.sort(cellsForBasalSegments))
...
#       print "potentialOverlaps for learningSegments"
#       print potentialOverlaps[learningSegments]
...
#     print "growthCandidates"
#     print growthCandidates
#     print "learningSegments"
#     print learningSegments
#     print "maxNew"
#     print maxNew
#     print "Segments belong to cells:"
#     print connections.mapSegmentsToCells(learningSegments)
...
class ApicalTiebreakSequenceMemory(ApicalTiebreakTemporalMemory):
  ...
  def compute2(self,
              ...
              learn=True):
    """
    A little different. Apical comes in the same timestep
...
#Cross column 
class CrossColumnApicalTiebreakSequenceMemory(ApicalTiebreakTemporalMemory):
  """
  Cross column Sequence memory with apical tiebreak.
  In this case the basal input comes from neighboring columns.
  We changed the context to be from the same timestep.
  """
  ...
  def __init__(self,
    ...
    super(CrossColumnApicalTiebreakSequenceMemory, self).__init__(**params)

    self.prevApicalInput = np.empty(0, dtype="uint32")
    self.prevApicalGrowthCandidates = np.empty(0, dtype="uint32")
    self.prevBasalInput = np.empty(0, dtype="uint32")
    self.prevbasalGrowthCandidates = np.empty(0, dtype="uint32")    
    self.prevPredictedCells = np.empty(0, dtype="uint32")
  ...
  def compute(self,
    ...
    self.activateCells(activeColumns, self.prevBasalInput, self.prevApicalInput,
                       self.prevBasalInput, self.prevApicalGrowthCandidates, learn)
    self.depolarizeCells(basalInput, apicalInput, learn)

    self.prevApicalInput = apicalInput.copy()
    self.prevApicalGrowthCandidates = apicalGrowthCandidates.copy()
    self.prevBasalInput = basalInput.copy()
    self.prevBasalGrowthCandidates = basalGrowthCandidates.copy()
